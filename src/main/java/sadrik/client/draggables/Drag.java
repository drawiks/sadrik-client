package sadrik.client.draggables;
...
import sadrik.util.animations.SweepAnim;
...
public class Drag {
    ...
    public static void onDraw(DrawContext context, int mouseX, int mouseY, float delta, boolean isChatScreen) {
        ...
        if (isChatScreen && draggingElement != null) {
            draggingElement.setX((int) (mouseX - startX));
            draggingElement.setY((int) (mouseY - startY));
        }
        ...
        if (click.button() == 0) {
            ...
            if (element != null) {
                if (element instanceof AbstractHudElement abstractElement && abstractElement.isDraggable()) {
                    ...
                    startX = mouseX - element.getX();
                    startY = mouseY - element.getY();
                }
            }
        }
    }

    public static void onMouseDrag(Click click) {
        if (draggingElement != null) {
            double mouseX = click.x();
            double mouseY = click.y();
            draggingElement.setX((int) (mouseX - startX));
