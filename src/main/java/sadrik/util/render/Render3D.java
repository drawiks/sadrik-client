...
public class Render3D implements IMinecraft {
    private static final int SHAPE_CACHE_MAX = 5000;
    private final Map<VoxelShape, Pair<List<Box>, List<Line>>> SHAPE_OUTLINES = Collections.synchronizedMap(
            new LinkedHashMap<>(16, 0.75f, true) {
                @Override
                protected boolean removeEldestEntry(Map.Entry<VoxelShape, Pair<List<Box>, List<Line>>> eldest) {
                    return size() > SHAPE_CACHE_MAX;
                }
            });
    private final Map<VoxelShape, List<Box>> SHAPE_BOXES = Collections.synchronizedMap(
            new LinkedHashMap<>(16, 0.75f, true) {
                @Override
                protected boolean removeEldestEntry(Map.Entry<VoxelShape, List<Box>> eldest) {
                    return size() > SHAPE_CACHE_MAX;
                }
            });
    ...
    public final List<Quad> QUAD_THROUGH = new ArrayList<>();
    public final List<Line> LINE_THROUGH = new ArrayList<>();
    ...
    public void onWorldRender(WorldRenderEvent e) {
        ...
        renderQuadsThrough(matrices, immediate, cameraPos);
        renderLinesThrough(matrices, immediate, cameraPos);
    ...
    private void renderLinesThrough(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, Vec3d cameraPos) {
        if (LINE_THROUGH.isEmpty()) return;

        VertexConsumer buffer = immediate.getBuffer(ClientPipelines.BLOCK_ESP_LINE);

        for (Line line : LINE_THROUGH) {
            drawLineVertex(matrices, buffer, line, cameraPos);
        }
    ...
    private void renderQuadsThrough(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, Vec3d cameraPos) {
        if (QUAD_THROUGH.isEmpty()) return;

        VertexConsumer buffer = immediate.getBuffer(ClientPipelines.BLOCK_ESP_QUAD);

        for (Quad quad : QUAD_THROUGH) {
            drawQuadVertex(matrices, buffer, quad, cameraPos);
        }
    ...
    public void drawBoxThroughBlocks(Box box, int color, float width) {
        drawBoxThroughBlocks(null, box, color, width, true, true);
    }

    public void drawBoxThroughBlocks(MatrixStack.Entry entry, Box box, int color, float width, boolean line, boolean fill) {
        double x1 = box.minX;
        double y1 = box.minY;
        double z1 = box.minZ;
        double x2 = box.maxX;
        ...
        if (fill) {
            int fillColor = ColorUtil.multAlpha(color, 0.3f);
            drawQuadThrough(entry, new Vec3d(x1, y1, z1), new Vec3d(x2, y1, z1), new Vec3d(x2, y1, z2), new Vec3d(x1, y1, z2), fillColor);
            drawQuadThrough(entry, new Vec3d(x1, y1, z1), new Vec3d(x1, y2, z1), new Vec3d(x2, y2, z1), new Vec3d(x2, y1, z1), fillColor);
            drawQuadThrough(entry, new Vec3d(x2, y1, z1), new Vec3d(x2, y2, z1), new Vec3d(x2, y2, z2), new Vec3d(x2, y1, z2), fillColor);
            drawQuadThrough(entry, new Vec3d(x1, y1, z2), new Vec3d(x2, y1, z2), new Vec3d(x2, y2, z2), new Vec3d(x1, y2, z2), fillColor);
            drawQuadThrough(entry, new Vec3d(x1, y1, z1), new Vec3d(x1, y1, z2), new Vec3d(x1, y2, z2), new Vec3d(x1, y2, z1), fillColor);
            drawQuadThrough(entry, new Vec3d(x1, y2, z1), new Vec3d(x1, y2, z2), new Vec3d(x2, y2, z2), new Vec3d(x2, y2, z1), fillColor);
        }

        if (line) {
            drawLineThrough(entry, x1, y1, z1, x2, y1, z1, color, width);
            drawLineThrough(entry, x2, y1, z1, x2, y1, z2, color, width);
            drawLineThrough(entry, x2, y1, z2, x1, y1, z2, color, width);
            drawLineThrough(entry, x1, y1, z2, x1, y1, z1, color, width);
            drawLineThrough(entry, x1, y1, z2, x1, y2, z2, color, width);
            drawLineThrough(entry, x1, y1, z1, x1, y2, z1, color, width);
            drawLineThrough(entry, x2, y1, z2, x2, y2, z2, color, width);
            drawLineThrough(entry, x2, y1, z1, x2, y2, z1, color, width);
            drawLineThrough(entry, x1, y2, z1, x2, y2, z1, color, width);
            drawLineThrough(entry, x2, y2, z1, x2, y2, z2, color, width);
            drawLineThrough(entry, x2, y2, z2, x1, y2, z2, color, width);
            drawLineThrough(entry, x1, y2, z2, x1, y2, z1, color, width);
        }
    }

    private void drawQuadThrough(MatrixStack.Entry entry, Vec3d x, Vec3d y, Vec3d w, Vec3d z, int color) {
        QUAD_THROUGH.add(new Quad(entry, x, y, w, z, color));
    }

    private void drawLineThrough(MatrixStack.Entry entry, double minX, double minY, double minZ, double maxX, double maxY, double maxZ, int color, float width) {
        LINE_THROUGH.add(new Line(entry, new Vec3d(minX, minY, minZ), new Vec3d(maxX, maxY, maxZ), color, color, width));
    }
